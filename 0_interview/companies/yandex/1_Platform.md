...
<details>
<summary>Реализовать функцию curry (каррирование)</summary>
...
Стоит спросить а внутри fn - это промис верно?

```js
const timeLimited = (fn, t) => {
  const errorMessage = "Time Limit Exceeded";
  // Функция исполнитель, которая будет использовано для создания promise. 
  // Она будет использовать setTimeout, чтобы через время t отклонить promise с сообщением, которая мы передаем.
