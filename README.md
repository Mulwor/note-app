...
<details>
<summary>Почему вы решили покинуть компанию (ищите новое место)?</summary>
...
<details>
<summary>Расскажи про текущий проект, сколько у вас человек, что за продукт вы разрабатываете, что ты делаешь на фронте что успешного смог реализовать за это время</summary>

Здравствуйте меня зовут Али Адыгезалли, я фронтенд-разработчик с опытом более 5 лет. Если говорить о нынешнем месте моей работы, а именно farsight management то я работаю над 2 проектами: это букинг-движок для бронирования отелей и вилл в Индонезии и на Бали и внутренняя платформа для персонала. Там собрана вся информация по заезду: ФИО гостя, сроки проживания, статус уборки номера и аналитика популярности того или иного номера

Если говорить о моем вкладе, то здесь два основных достижения.

- В букинг-движке я полностью переписал старый легаси-проект на современный стек, так как предыдущая версия сильно тормозила и имела проблемы с отзывчивостью интерфейса, что в свою очередь увеличило с 3 заходов на сайт в месяц до 500 человек в месяц согласно аналитикам;
- А для внутренней платформы я с нуля разработал систему календарей. Она была в двух форматах — линейном и месячном. По вертикали там указаны номера, по горизонтали — даты заезда и выезда. Это сделало управление бронями гораздо нагляднее.

До этого работал в ИТМО и наиболее интересными могу выделить следующие проекты/задачи в которых я принимал участие:

- Первый - это visual website builder, где пользователи (преподаватели) могли собирать страницы из готовых блоков вроде хедера и футера и редактировать их. Я реализовал внутри этого проекта - встроенный текстовый редактор с расширенными возможностями форматирования: жирный, курсив, гиперссылки, тултипы, надстрочный и подстрочный текст.
...
<details>
<summary>Вы целенаправленно откликнулись на нашу вакансию? / Чем вас заинтересовало наша вакансий / Чего к нам перевело?</summary>
...
<details>
<summary>Какая ЗП у вас на текущей работе / Какие у вас зарплатные ожидания?</summary>
...
<details>
<summary>Расскажите о самых сложных проблемах, над которыми вы работали. Расскажите в подробностях, как вы их решали</summary>
...
<img src="./0_interview/assets/html/table.PNG" alt="table" />
...
<details>
<summary>GIT - система контроля версий для отслеживания изменений в коде</summary>
...
4. `Object.fromEntries()` - у нас есть многомерный массив и с помощью данного метода мы можем преобразовать его в объект с ключом и значением
...
3. В обычных функция this - динамический (в зависимости от того, где мы его вызываем, на то он и будет ссылаться), а у стрелочных — лексический (определяется при объявлении).
...
<h3 align="center">Задачи с this</h3>
<details>
...
Ответ: Если говорим про браузер, то в строгом и нестрогом режиме покажет object window. А если в терминале, то это object global
...
- S `(single responsibility principle)` - принцип единственной ответственный. Один класс, компонент должен выполнять лишь 1 задачу (ответственность)

- O `(open-closed principle)` - принцип открытости и закрытости. Код написанные на классах, компонентов должен быть открыт для расширения, но закрыт для изменения.
...
- L `(liskov substitution principle)` - принцип подставки Барбары Лисков. Наследуемый класс, компонент должен расширять, а не заменять поведение базового (родительского) класса

- I `(interface segregation principle)` - принцип разделения интерфейса. Принцип гласит, что не стоит хранить все в одном месте и необходимо разбивать код на более мелкие компоненты (классы/интерфейсы) связанные по логике
...
<details>
<summary>Что такое TypeScript, зачем он нужен и какие преимущество и недостатки у него?</summary>
...
type ValueOf<T> =
  T extends Arr<infer U> ? U : T extends Obj ? T[keyof T] : never;
```
...
3. `GET`-запросы у нас кешируется, а `POST`-запросы нет.
