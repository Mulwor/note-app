...
- [Модули в Node.js](#модули-в-nodejs)
  ...
  - [Module core (модульная система)](#module-system)
    ...
    - [http/https](#module-system-http)
      - [Создание базового сервера, модуль http и req, res](#createServer)
      - [Создание полноценного сервера на чистом node-js](https://github.com/Mulwor/nodejs-crud-api/blob/dev/src/index.ts)
      - [Routing на сервере и передача статических методов](#routingOnServer);
...
Существует несколько способов взаимодействия с путями, например мы можем относительно вызвать его `./files/fileToRead.txt`, но это плохая практика, так как на разных операционных систем используется разные виды слешов. Например в линуксе есть обратный слеш `\`, и это надо учитывать. `В начале нам необходимо импортировать import path from 'path' либо использовать деструктуризацию import {resolve} from 'path' как например`


|Метод|Что делает|Когда использовать|
|----|-----|----|
|`.join('files', 'fileToRead.txt')` | Позволяет склеивать несколько участков пути, используя правильный разделитель для ОС. => files/fileToRead.txt. Он работает только с относительными путями, не создает абсолютный путь. У него нет начальной точки | 1. Нужно просто объединить части относительного пути; <br /> 2. Создаете путь для импорта модулей внутри проекта;<br/> 3. Работаете с путями конфигурации, которые будут использоваться относительно; |
|`.resolve('src', 'app.js')` | Создает абсолютный путь => '/full/path/to/current/dir/src/app.js' и всегда возвращает его. Начальная точка его - относительно текущей рабочей директории | 1. для файловых операций; <br /> 2. работаете с путями от пользовательского ввода <br /> 3. Нужно нормализовать путь с .. и . <br /> 4. Определяете корневые директории | 
|`.basename(pathToFile, withoutExtension)` | Получает имя файла с расширением, вторым аргументом передаем если хотим это расширения убрать | [1. Когда хотим получить имя файла из полного пути; 2. Когда хотим создать копию файла с сохранением имени; 3. Для создания уникальных имен файла; 4. Для валидации файлов](./path/basename.js) | path.basename('/path/to/file.txt');  => file.txt <br/> path.basename('/path/to/file.txt', '.txt'); => file |
| `.dirname(pathToDirectory)` | Получаем директорию из `/path/to/file.txt` получаем `path/to` | - |
| `.extname(pathToExtensionFile)` | Получаем расширение файла - `.txt` | - | 
| `.parse()` | [Разбирает путь на составные части](./path/parse/1.js) | Когда вам нужно получить несколько или все компоненты пути одновременно или сложно модифицировать путь, сохраняя его структуру. Это избавляет от многократных вызовов dirname(), basename(), extname() и делает код чище. |  
| `.normalize(pathToFile)` | Убирает лишние символы - когда пользовать условно в консоле написал два слеша вместо одной или вообще путь неправильно написал | [1. Когда понимает что пользователь может случайно ввести ненужные символы; 2. Когда хотим сравнить две пути ведущие к одному файлу; 3. при работе с конфигурационными файлами](./path/normalize.js) | 
| `.isAbsolute(pathToFile)` | Проверяем является ли путь абсолютным => true / false | - | 
...
|`.chdir(..)`| Изменяет текущию директорию, например идет на уровень выше cd .. |
