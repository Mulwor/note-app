...
`Условные типы` - он позволяют выбирать тип на основе условия. С помощью условного типа мы можем убрать повторяющийся код, обработать разные случае
...
`Generic` (обобщенный тип) - механизм в TypeScript, который позволяет писать более гибкую типизацию, динамически устанавливать типы, а также решает проблему избыточного кода, когда с помощью одного дженерика мы можем удалить много похожих функций.
